...
import { Input, Choice, SelectChoice } from './InputEnquirer';

type HorrorsMovieInput = {
  title: string;
  director: string;
  language: string;
  running_time: string;
...
export class Main {
  ...
  async incrementJumpScares() {
    ...
    this.movies.forEach((movie: Movie) => {
      if (movie.getId === idOfMovieToIncrementJumpScares) {
        if (movie instanceof HorrorMovie) return movie.incrimentJumpScare();
      }
    });
  ...
  async incrementExplosions() {
    ...
    this.movies.forEach((movie: Movie) => {
      if (movie.getId === idOfMovieToIncrementExplosions) {
        if (movie instanceof ActionMovie) return movie.incrimentExplotions();
      }
    });
